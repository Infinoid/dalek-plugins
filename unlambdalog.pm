...
sub init {
    my $self = shift;
    $copy_of_self = $self;
    my $rev = main::get_item($self, "unlambda_lastrev");
    undef $rev unless length $rev;
    $lastrev = $rev if defined $rev;
    main::lprint("unlambdalog: init: initialized lastrev to $lastrev") if defined $lastrev;
    main::create_timer("unlambdalog_fetch_feed_timer", $self, "fetch_feed", 30);
}
...
sub shutdown {
    my $self = shift;
    main::delete_timer("unlambdalog_fetch_feed_timer");
    main::store_item($self, "unlambda_lastrev", $lastrev) if defined $lastrev;
}
...
sub process_feed {
    ...
    $lastrev = $latest;
    main::store_item($copy_of_self, "unlambda_lastrev", $lastrev);
}
...
sub output_item {
    ...
    return main::lprint("unlambdalog: error parsing filenames from description")
        unless $lines[0] eq '';
    shift(@lines);
    ...
    $rev = substr($rev, 0, 7);
    put("unlambda: $rev | $creator++ | $prefix:");
    foreach my $line (@log_lines) {
	put("unlambda: $line");
    }
    put("unlambda: review: $link");
    main::lprint("unlambdalog: output_item: output rev $rev");
}
