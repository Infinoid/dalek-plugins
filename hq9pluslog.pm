...
sub init {
    my $self = shift;
    $copy_of_self = $self;
    my $rev = main::get_item($self, "hq9plus_lastrev");
    undef $rev unless length $rev;
    $lastrev = $rev if defined $rev;
    main::lprint("hq9pluslog: init: initialized lastrev to $lastrev") if defined $lastrev;
    main::create_timer("hq9pluslog_fetch_feed_timer", $self, "fetch_feed", 30);
}
...
sub shutdown {
    my $self = shift;
    main::delete_timer("hq9pluslog_fetch_feed_timer");
    main::store_item($self, "hq9plus_lastrev", $lastrev) if defined $lastrev;
}
...
sub process_feed {
    ...
    $lastrev = $latest;
    main::store_item($copy_of_self, "hq9plus_lastrev", $lastrev);
}
...
sub output_item {
    ...
    return main::lprint("hq9pluslog: error parsing filenames from description")
        unless $lines[0] eq '';
    shift(@lines);
    ...
    $rev = substr($rev, 0, 7);
    put("hq9plus: $rev | $creator++ | $prefix:");
    foreach my $line (@log_lines) {
	put("hq9plus: $line");
    }
    put("hq9plus: review: $link");
    main::lprint("hq9pluslog: output_item: output rev $rev");
}
