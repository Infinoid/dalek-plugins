...
sub init {
    my $self = shift;
    $copy_of_self = $self;
    my $rev = main::get_item($self, "eclectus_lastrev");
    undef $rev unless length $rev;
    $lastrev = $rev if defined $rev;
    main::lprint("eclectuslog: init: initialized lastrev to $lastrev") if defined $lastrev;
    main::create_timer("eclectuslog_fetch_feed_timer", $self, "fetch_feed", 30);
}
...
sub shutdown {
    my $self = shift;
    main::delete_timer("eclectuslog_fetch_feed_timer");
    main::store_item($self, "eclectus_lastrev", $lastrev) if defined $lastrev;
}
...
sub process_feed {
    ...
    $lastrev = $latest;
    main::store_item($copy_of_self, "eclectus_lastrev", $lastrev);
}
...
sub output_item {
    ...
    return main::lprint("eclectuslog: error parsing filenames from description")
        unless $lines[0] eq '';
    shift(@lines);
    ...
    $rev = substr($rev, 0, 7);
    put("eclectus: $rev | $creator++ | $prefix:");
    foreach my $line (@log_lines) {
	put("eclectus: $line");
    }
    put("eclectus: review: $link");
    main::lprint("eclectuslog: output_item: output rev $rev");
}
