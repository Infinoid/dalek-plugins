package modules::local::gitoriusparser;
use strict;
use warnings;
...
=head1 NAME
    ...
    modules::local::gitoriusparser

=head1 DESCRIPTION

This module is responsible for parsing ATOM feeds generated by gitorius.org.
...
sub try_link {
    ...
    my $project;
    if($url =~ m|http://gitorius.org/([^/]+)/?|) {
        $project = $1;
    } else {
        # whatever it is, we can't handle it.  Log and return.
        main::lprint("gitorius try_link(): I can't handle $url");
        return;
    }
    ...
    my $rss_link = "http://gitorius.org/$project.atom";
    my $self = {
        url        => $rss_link,
        feed_name  => $project,
    ...
    eval "package $modulename; use base 'modules::local::gitoriusparser';";
    $objects_by_package{$modulename} = bless($self, $modulename);
    main::lprint("$parsername gitorius ATOM parser autoloaded.");
