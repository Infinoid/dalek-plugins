...
sub init {
    my $self = shift;
    $copy_of_self = $self;
    my $rev = main::get_item($self, "m4_lastrev");
    undef $rev unless length $rev;
    $lastrev = $rev if defined $rev;
    main::lprint("m4log: init: initialized lastrev to $lastrev") if defined $lastrev;
    main::create_timer("m4log_fetch_feed_timer", $self, "fetch_feed", 30);
}
...
sub shutdown {
    my $self = shift;
    main::delete_timer("m4log_fetch_feed_timer");
    main::store_item($self, "m4_lastrev", $lastrev) if defined $lastrev;
}
...
sub process_feed {
    ...
    $lastrev = $latest;
    main::store_item($copy_of_self, "m4_lastrev", $lastrev);
}
...
sub output_item {
    ...
    return main::lprint("m4log: error parsing filenames from description")
        unless $lines[0] eq '';
    shift(@lines);
    ...
    $rev = substr($rev, 0, 7);
    put("m4: $rev | $creator++ | $prefix:");
    foreach my $line (@log_lines) {
	put("m4: $line");
    }
    put("m4: review: $link");
    main::lprint("m4log: output_item: output rev $rev");
}
